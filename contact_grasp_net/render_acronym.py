...
from contact_grasp_net import config_parser, utils
from contact_grasp_net.datagen_scene_renderer import SceneRenderer
...
class AcronymRender():
    ...
    def render_scene(self, scene_id):
        ...
        obj_paths = [Path(p).parent.parent / Path(p).name for p in obj_paths]
        ...
        for i, cam_ori in enumerate(pbar):
            ...
            pc_cam, pc_normals, camera_pose, depth, color = self.render_pose(renderer, camera_pose, estimate_normals=self._estimate_normals)
            ...
            np.savez(save_pose_path, xyz=pc_cam[:,:3])
    
    def change_scene(self, renderer, obj_paths, obj_scales, obj_transforms, visualize=True):
        """
    ...
    def render_pose(self, renderer, camera_pose, estimate_normals=False):
        ...
        # 0.005 s
        color, depth, _, camera_pose = renderer.render(in_camera_pose, render_pc=False)
        ...
        return pc, pc_normals, camera_pose, depth, color
...
if __name__ == '__main__':
    ...
    global_config = config_parser.load_config(config_path="/home/raphael/thesis/contact_former/contact_grasp_net/config_original.yaml")

    render = AcronymRender(global_config)
    # render.render_acronym()

    render.render_scene('005251')