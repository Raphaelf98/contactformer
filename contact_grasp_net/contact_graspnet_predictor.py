...
from contact_grasp_net.data import farthest_points, \
...
class GraspPredictor:
    """
    Class for inferencing Contact GraspNet
    """
    def __init__(self, config, ContactGraspNet = None):
        if 'surface_grasp_logdir_folder' in config:
            # for sim evaluation
            self._contact_grasp_cfg = config_parser.load_config(config['surface_grasp_logdir_folder'], batch_size=1, arg_configs=config.arg_configs)
            self._cfg = config
            self._num_samples = self._cfg.num_samples
        else:
            self._contact_grasp_cfg = config
    ...
    def filter_segment(self,contact_pts,segment_pc, thresh=1e-5):
        """
        ...
        if contact_pts.shape[0] > 0 and segment_pc.shape[0] > 0:
            try:
                ...
                filtered_grasp_idcs = np.where(min_dists<thresh)
            except:
                pass
        ...
        if filter_grasps:
            ...
            for k in segment_keys:
                ...
                if np.any(pc_segments[k]) and np.any(contact_pts[j]):
                    segment_idcs = self.filter_segment(contact_pts[j], pc_segments[k], thresh=self._contact_grasp_cfg['TEST']['filter_thres'])
