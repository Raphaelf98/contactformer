...
from contact_grasp_net.mesh_utils import Object
...
class SceneRenderer:
    ...
    def __init__(self, intrinsics=None, fov=np.pi / 6, caching=True, viewing_mode=False):
        ...
        if viewing_mode:
            self._viewer = pyrender.Viewer(self._scene, use_raymond_lightning=True, run_in_thread=True)
    ...
    def _init_camera_renderer(self):
        """
        If not visualizing mode, inititalize camera with given intrinsics
        ...
        if self._intrinsics in ['kinect_azure', 'realsense']:
            ...
            self.renderer = pyrender.OffscreenRenderer(viewport_height=self._height,viewport_width=self._width, point_size=1.0)
        else:
