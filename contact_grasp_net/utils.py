...
def build_6d_grasp(approach_dirs, base_dirs, contact_pts, thickness, use_torch=False, gripper_depth = 0.1034, device=None):
    ...
    if use_torch:
        ...
        assert contact_pts.dim() == 3, f"Expected 3D tensor of shape (B, N, 3), got {contact_pts.shape}"
        assert contact_pts.shape[0] > 0, f"Batch size must be > 0, got {contact_pts.shape[0]}"
        assert contact_pts.shape[1] > 0, f"Number of points must be > 0, got {contact_pts.shape[1]}"
        assert contact_pts.shape[2] == 3, f"Last dimension must be 3 (XYZ coordinates), got {contact_pts.shape[2]}"
